...
class test_heketi(unittest.TestCase):
    ...
    def test_dirvolume(self):
        ...
        # Export dirvolume with a bad id
        dirvolume_ex_params = {}
        dirvolume_ex_params['iplist'] = ['10.0.0.1','10.0.0.2']

        with self.assertRaises(requests.exceptions.HTTPError):
            c.dirvolume_export("badid", dirvolume_ex_params)

        # Export dirvolume
        print ("Exporting dirvolume")
        dirvolumeInfo = c.dirvolume_export(dirvolume['id'], dirvolume_ex_params)
        self.assertEqual(len(dirvolumeInfo['iplist']), 2)

        # Unexport dirvolume with a bad id
        dirvolume_ex_params = {}
        dirvolume_ex_params['iplist'] = ['10.0.0.2']

        with self.assertRaises(requests.exceptions.HTTPError):
            c.dirvolume_unexport("badid", dirvolume_ex_params)

        # Unexport dirvolume
        print ("Unexporting dirvolume")
        dirvolumeInfo = c.dirvolume_unexport(dirvolume['id'], dirvolume_ex_params)
        self.assertEqual(len(dirvolumeInfo['iplist']), 1)
